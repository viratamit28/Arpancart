...
const Cart = () => {
  // Context se cart ka data nikal rahe hain
  const { cartItems, addToCart } = useContext(CartContext);

  // Total amount calculate karne ka logic
            ...
            {cartItems.map((item, index) => (
              ...
              >
                ...
                <div className="flex-grow text-center sm:text-left">
                  ...
                  {/* Quantity & Actions (Basic design, aage chal ke increment/decrement laga sakte ho) */}
                  <div className="flex items-center justify-center sm:justify-start gap-4 mt-4">
                    <div className="flex items-center border border-gray-200 rounded-md">
                      <span className="px-4 py-1.5 font-bold text-gray-700 bg-gray-50 border-r border-gray-200">Qty</span>
                      <span className="px-4 py-1.5 font-bold text-[#8b1818]">{item.quantity}</span>
                    </div>
                  </div>
                </div>

                {/* Subtotal for this item */}
                <div className="sm:text-right w-full sm:w-auto mt-4 sm:mt-0 border-t sm:border-none pt-4 sm:pt-0">
